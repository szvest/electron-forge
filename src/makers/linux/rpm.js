...
import linuxConfig, { populate } from '../../util/linux-config';
...
export default async ({ dir, targetArch, forgeConfig, packageJSON }) => {
  ...
  const config = populate({ forgeConfig, configKey: 'electronInstallerRedhat', targetArch });
  const name = config.maker.options.name || config.shared.options.name || packageJSON.name;
